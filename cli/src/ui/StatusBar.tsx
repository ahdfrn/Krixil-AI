/**
 * A persistent context bar for the interactive REPL — real tool-call count and real
 * working-tree change stats for the current run, not fabricated "context usage" or "tokens"
 * figures Krixil doesn't track. Change stats reuse checkpoint.ts's own git plumbing
 * (workingTreeChangeSummary) so this and `kirxil undo` never disagree about what "changed" means.
 */
...
export function StatusBar({ toolCalls, testAttempts }: { toolCalls: number; testAttempts: number }) {
  ...
  if (testAttempts > 0) parts.push(`${testAttempts} test attempt${testAttempts === 1 ? "" : "s"}`);
